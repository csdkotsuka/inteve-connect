...
export default function AuthModal({ isOpen, onAuthenticated, patientType: initialPatientType = 'new', industryType = 'medical' }) {
  ...
  const handleSelectProvider = async (provider) => {
    ...
    if (provider === 'line') {
      ...
      const patientPhone = isRet ? (formData.phone || '090-7549-8513') : (formData.phone || '090-0000-0000');
      let result = null;
      if (isRet) {
      ...
      const patientPhone = isRet ? (formData.phone || '080-1234-5678') : (formData.phone || '090-1111-2222');
      let result = null;
      if (isRet) {
        result = await matchPatient(patientName, patientPhone);
      }
      ...
      const patientPhone = isRet ? (formData.phone || '090-7549-8513') : (formData.phone || '090-3333-4444');
      let result = null;
      if (isRet) {
        result = await matchPatient(patientName, patientPhone);
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      ...
      >
        ...
        {/* 顧客/患者区分セレクター（新規 / 再来） */}
        <div className="bg-slate-50 px-6 py-4 border-b border-slate-200/80 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className={`px-5 py-2 text-sm rounded-full font-bold transition-all cursor-pointer ${
                patientType === 'new'
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              <button
                ...
                className="p-4 rounded-2xl border border-slate-200 hover:border-[#06C755] bg-white hover:bg-emerald-50/40 flex flex-col items-center justify-center gap-2 transition-all text-sm font-bold cursor-pointer active:scale-95 shadow-sm"
              >
              ...
              <button
                ...
                className="p-4 rounded-2xl border border-slate-200 hover:border-black bg-white hover:bg-slate-50 flex flex-col items-center justify-center gap-2 transition-all text-sm font-bold cursor-pointer active:scale-95 shadow-sm"
              >
              ...
              <button
                ...
                className="p-4 rounded-2xl border border-slate-200 hover:border-blue-500 bg-white hover:bg-blue-50/40 flex flex-col items-center justify-center gap-2 transition-all text-sm font-bold cursor-pointer active:scale-95 shadow-sm"
              >
              ...
              <button
                ...
                className={`p-4 rounded-2xl border flex flex-col items-center justify-center gap-2 transition-all text-sm font-bold cursor-pointer active:scale-95 shadow-sm ${
                  authMethod === 'email'
          ...
          <AnimatePresence>
            {authMethod === 'email' && (
              ...
              >
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-medium"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-medium font-mono"
                    />
                  </div>
                </div>
                ...
                {/* 新患の案内 */}
                {patientType === 'new' && (
                  <div className="p-3.5 rounded-xl bg-emerald-50 text-emerald-900 border border-emerald-200 text-sm flex items-center gap-2.5">
                    <CheckCircle2 size={18} className="text-emerald-600 shrink-0" />
                    <span>{labels.firstVisit}（新規アカウントを作成します）</span>
                  </div>
                )}

                {/* 再診/リピートで照合できた場合のバッジ */}
                {patientType === 'returning' && matchResult?.isReturning && matchResult.customerCode && (
                  <div className="p-3.5 rounded-xl bg-blue-50 text-blue-900 border border-blue-200 text-sm flex items-center gap-2.5">
                ...
                <button
                  ...
                  className="w-full py-4 bg-slate-900 hover:bg-black text-white font-bold rounded-xl text-base shadow-lg transition-all cursor-pointer disabled:opacity-40 flex items-center justify-center gap-2 mt-2"
                >
